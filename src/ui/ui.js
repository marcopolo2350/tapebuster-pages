...
export class UI {
  ...
  /**
   * "You are now listening to …" — shown on every start, switch and resume.
   *
   * The dial moves BY ITSELF now: when a station runs out of records it hands
   * over to the next one. A shopper facing a shelf has no view of the dock, so
   * without this the music would simply become different music for no visible
   * reason. It is an announcement over a game, not an interruption of one —
   * no pointer events, no focus, no pause, and it takes itself away.
   */
  announceStation(label) {
    const el = $('radio-banner');
    if (!el || !label) return;
    el.textContent = '';
    ...
    line.append('You are now listening to ');
    const b = document.createElement('b');
    // textContent throughout: station names are ours, but building UI out of
    // string concatenation is how markup injection arrives later.
    b.textContent = label;
    line.appendChild(b);
    el.append(eq, line);
    el.classList.remove('hidden');
    // Re-announcing while one is already up must restart the dwell, not stack
    // two timers that race to hide it.
    clearTimeout(this._bannerIn);
    clearTimeout(this._bannerOut);
    this._bannerIn = setTimeout(() => el.classList.add('show'), 20);
    this._bannerOut = setTimeout(() => {
