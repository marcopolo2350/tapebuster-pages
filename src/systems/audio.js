...
import { DEFAULT_STATION, STATION_BY_ID, YT_STATIONS } from './radio/stations-yt.js';
...
export class StoreAudio {
  ...
  /**
   * What the dial points at, known LOCALLY.
   *
   * `source` is what gets handed to the player, and it comes in two shapes on
   * purpose: the store's own stations are ARRAYS OF RECORDINGS we chose, while
   * a playlist the shopper pasted in is still a YouTube PLAYLIST ID. Their
   * list is theirs; rebuilding ours was never a reason to break it.
   */
  stationPlaylists(id = this.ytStation()) {
    const override = this.ytPlaylist();
    if (override) return { id: 'custom', label: 'CUSTOM', source: override };
    if (id === 'custom') {
      const cur = customPlaylists().find((p) => p.id === localStorage.getItem('tb_custom_current'));
      return cur ? { id: 'custom', label: cur.name, source: cur.playlistId } : null;
    }
    const st = STATION_BY_ID[id];
    return st ? { id: st.id, label: st.label, source: st.tracks ?? [] } : null;
  }
  ...
  async prewarmRadio() {
    ...
    try {
      ...
      this._yt = new YouTubeRadio({
        onChange: () => this.onRadioChange?.(),
        onStationEnd: (from) => this.advanceStation(from),
        volume: this.radioVol,
      });
      await this._yt.prewarm({ playlistId: typeof sel?.source === 'string' ? sel.source : null, stationId: sel?.id ?? null });
    } catch (e) {
  ...
  playStation(id) {
    ...
    if (!sel || (Array.isArray(sel.source) && !sel.source.length)) {
      this._trouble = 'That station has no records on it.'; this.onRadioChange?.(); return false;
    }
    this._trouble = null;
    if (!this._yt) { this.prewarmRadio(); this._trouble = 'Preparing radio…'; this.onRadioChange?.(); return false; }
    // ANNOUNCE ON EVERY START, SWITCH AND RESUME. The dial can change on its
    // own now, so a shopper who is looking at a shelf rather than the dock has
    // no other way to know the station moved.
    this.onStationAnnounce?.(sel.label);
    return this._yt.playStation(sel.id, sel.source);
  }
  ...
  /**
   * THE STATION FINISHED, SO THE NEXT ONE STARTS.
   *
   * A dial that stops dead on its last record makes the shopper walk back to
   * Settings to hear anything else, which is not what a radio does. The order
   * is the dial's own order, and the end wraps to the front so the store never
   * falls silent on its own.
   *
   * NOT called from a gesture — it fires when a track ends — so the browser
   * may refuse the next play. That is fine and already handled: the player
   * watches for a refused start and the dock offers TAP TO PLAY. Advancing
   * anyway is right, because the alternative is silence with no explanation.
   */
  advanceStation(fromId = null) {
    const ids = YT_STATIONS.map((s) => s.id);
    if (!ids.length) return false;
    const cur = fromId ?? this.ytStation();
    const at = ids.indexOf(cur);
    const next = ids[(at + 1 + ids.length) % ids.length];
  ...
  startRadio() {
    ...
    const sel = this.stationPlaylists();
    if (sel) this.onStationAnnounce?.(sel.label);
    if (this._yt.lastState) return this._yt.play();
    return sel ? this._yt.playStation(sel.id, sel.source) : false;
  }
