...
function depunct(s) {
  ...
  //
  // '&' BECOMES THE WORD, because it is a word. Measured over the stocked
  // 20,000: 240 titles carry an ampersand, and asked the way people type them
  // — "fast and furious", "mr. and mrs. smith" — NONE of them resolved.
  // 208 were FLATLY DENIED: the clerk said the store did not have a film that
  // was sitting on a shelf. The reverse was worse still, because it is the
  // commoner ask: of 120 titles spelled "and", asked with "&", 100 were
  // denied. titleMatchScore('Fast & Furious', 'fast and furious') was 0.
  //
  // It belongs HERE rather than in a special case because this function is
  // already the place where the two spellings of a punctuation mark are
  // compared away in both directions — the same rule that lets "mission
  // impossible" reach 'Mission: Impossible'. Both sides converge on "and", so
  // it does not matter which way round the shopper and the catalogue disagree.
